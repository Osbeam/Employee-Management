  ...
  useEffect(() => {
    ...
    if (selectedOption) {
      const filtered = data.filter(
        (item) => item.DatabaseOwner === selectedOption
      );
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await fetch(
        "http://77.37.45.224:8000/api/admin/upload",
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
    <>
      ...
      <div className="dop_container">
        <a
          className="downloadtemplete"
          href="https://docs.google.com/spreadsheets/d/1zUUTTEbqAXAgsBmkBQSMRnWinDoSEfP6/export?format=xlsx"
          download
        >
          Download Template
        </a>
      ...
      <div className="table-dop-container">
        <div>
          <select
            className="dop-dropdown"
            onChange={handleSelectChange}
            value={selectedOption}
          >
            <option className="drop-option" value="">
              Select
            </option>
            {Array.from(new Set(data.map((item) => item.DatabaseOwner))).map(
              (owner) => (
          ...
          <Row gutter={16}>
            ...
            <Col span={6}>
              <Form.Item label="Existing Loan Amount">
                <Input
                  name="ExistingLoanAmt"
                  value={formData.ExistingLoanAmt}
                  onChange={handleInputChange}
                />
              </Form.Item>
            </Col>
