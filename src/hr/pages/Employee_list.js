...
export default function Employee_List() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await axios.get(
        `http://77.37.45.224:8000/api/user/getEmployee`,
        {
          params: {
  ...
  const filteredEmployees = employees.filter((employee) => {
    ...
    const designation = employee.Designation ? employee.Designation.name.toLowerCase() : '';
    const role = employee.Role ? employee.Role.join(', ').toLowerCase() : '';

    return (
      fullName.includes(searchParams.search.toLowerCase()) ||
      designation.includes(searchParams.search.toLowerCase()) ||
      role.includes(searchParams.search.toLowerCase())
    );
  });
